...
export default function ProductCard({ product, index = 0 }) {
  ...
  const delay = 0;
  const isBestSeller = tags.includes('Popular') || tags.includes('Best Seller');
  const isNew = tags.includes('New');
  const isHot = tags.includes('Hot') || tags.includes('Sale');

  return (
    <Link href={productUrl}
      className="group block relative overflow-hidden depth-card rounded-2xl"
    ...
    >
      <div className="aspect-square relative overflow-hidden bg-black/30 rounded-t-2xl">
        <img
          ...
          alt={`Buy ${product.name} in Nepal - GameShop Nepal`}
        ...
        {isFlashSale && <FlashSaleBadge endTime={product.flash_sale_end} small={true} />}
        ...
        {!product.is_sold_out && !isFlashSale && (
          <div className="absolute top-2.5 right-2.5 flex flex-col gap-1">
            {isBestSeller && (
              <Badge className="bg-amber-500 text-black text-[10px] font-bold px-2 py-0.5 rounded-full shadow-lg">🔥 Popular</Badge>
            )}
            {isNew && (
              <Badge className="bg-blue-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-lg">✨ New</Badge>
            )}
            {isHot && !isBestSeller && (
              <Badge className="bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-lg">🏷️ Sale</Badge>
            )}
          </div>
        )}
      ...
      <div className="p-3 sm:p-3.5 relative">
        ...
        <div className="flex items-center justify-between mt-1.5">
          ...
          <span className="text-[10px] text-amber-500/70 font-medium">Buy →</span>
        </div>
      </div>
    </Link>
  );
