...
import { X, Search, User, Gift, Users, Smartphone, Star, Home, ShoppingCart, Package } from 'lucide-react';
...
export default function Navbar({ notificationBarHeight = 0 }) {
  ...
  const isAdminRoute = pathname.startsWith('/panelgsnadminbackend');
  ...
  return (
    <>
      ...
      <div
        ...
        data-testid="navbar"
      >
        <Link href="/" className="flex items-center mr-2" data-testid="nav-logo">
        ...
        {isSearchOpen ? (
          <form onSubmit={handleSearch} className="flex items-center gap-1">
            <input
              ...
              className="bg-white/10 border border-white/15 rounded-full px-3 py-1.5 text-sm text-white placeholder:text-white/40 focus:outline-none focus:border-amber-500/50 w-44"
              autoFocus
              data-testid="search-input"
            />
            <button type="submit" className="text-amber-500 hover:text-amber-400 p-1">
      ...
      <div className="md:hidden fixed left-0 right-0 z-40 bg-black/95 border-b border-white/[0.06]" style={{ top: notificationBarHeight }} data-testid="mobile-top-bar">
        <div className="flex items-center justify-between px-4 h-12">
          <Link href="/" className="flex-shrink-0" data-testid="nav-logo-mobile">
          ...
          <div className="flex items-center gap-1">
            {isSearchOpen ? (
              <form onSubmit={handleSearch} className="flex items-center gap-1">
                ...
                />
                <button type="submit" className="text-amber-500 p-1">
                  <Search className="h-4 w-4" />
                </button>
            ...
            <button onClick={() => setShowInstallModal(true)} className="text-white/50 p-2 rounded-full" data-testid="add-to-home-btn-mobile">
