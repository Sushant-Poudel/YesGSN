...
import { 
  ...
  Store, Megaphone, Palette, Wrench, ClipboardList, Image, Smartphone, Volume2, VolumeX, Globe, MessageCircle, AlertTriangle
} from 'lucide-react';
...
const navGroups = [
  {
    ...
    items: [
      ...
      { path: '/panelgsnadminbackend/complaints', label: 'Complaints', icon: AlertTriangle, permission: 'view_complaints' },
      { path: '/panelgsnadminbackend/blocklist', label: 'Blocklist', icon: Shield, permission: 'view_orders' },
  ...
  },
  {
    ...
    items: [
      ...
      { path: '/panelgsnadminbackend/payment-redirect', label: 'Payment Redirect', icon: MessageCircle, permission: 'view_settings' },
      { path: '/panelgsnadminbackend/floating-button', label: 'Floating Button', icon: MessageCircle, permission: 'view_settings' },
...
export default function AdminLayout({ children, title }) {
  ...
  const [soundEnabled, setSoundEnabled] = useState(() => typeof window !== 'undefined' ? localStorage.getItem('admin_sound') !== 'off' : true);
  ...
  useEffect(() => {
    ...
    const api = axios.create({ baseURL: process.env.NEXT_PUBLIC_BACKEND_URL, headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleLogout = () => {
    ...
    router.push('/panelgsnadminbackend/login');
  };
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-black border-b border-white/10 px-4 py-3 flex items-center justify-between">
        <Link href="/"><img src={LOGO_URL} alt="GameShop Nepal" className="h-8" /></Link>
      ...
      <aside className={`admin-sidebar fixed left-0 top-0 bottom-0 w-64 flex flex-col z-50 transform transition-transform duration-300 lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`} data-testid="admin-sidebar">
        ...
        <nav className="flex-1 py-3 overflow-y-auto">
          {navGroups.map((group) => {
            ...
            const hasActiveItem = visibleItems.some(item => pathname === item.path);
