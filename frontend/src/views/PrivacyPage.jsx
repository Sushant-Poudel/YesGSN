...
export default function PrivacyPage({ initialData = null }) {
  const [pageData, setPageData] = useState(initialData || { title: 'Privacy Policy', content: '' });
  const [isLoading, setIsLoading] = useState(!initialData);

  useEffect(() => {
    pagesAPI.get('privacy').then(res => setPageData(res.data)).catch(() => {}).finally(() => setIsLoading(false));
  }, []);

  const defaultContent = `
    <h2>1. Introduction</h2>
    <p>Welcome to GameShop Nepal. By using our website and services, you agree to these privacy and conditions.</p>
