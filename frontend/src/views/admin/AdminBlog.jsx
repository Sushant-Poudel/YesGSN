...
const emptyPost = { title: '', slug: '', excerpt: '', content: '', image_url: '', cta_url: '', cta_text: 'Order Now', is_published: true };
...
export default function AdminBlog() {
  ...
  const handleOpenDialog = (post = null) => {
    if (post) { setEditingPost(post); setFormData({ title: post.title, slug: post.slug, excerpt: post.excerpt, content: post.content, image_url: post.image_url || '', cta_url: post.cta_url || '', cta_text: post.cta_text || 'Order Now', is_published: post.is_published }); }
          ...
          <DialogContent className="bg-card border-white/10 text-white max-w-2xl max-h-[90vh] overflow-y-auto sm:mx-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-2"><Label>CTA Button Text</Label><Input value={formData.cta_text} onChange={(e) => setFormData({ ...formData, cta_text: e.target.value })} className="bg-black border-white/20" placeholder="Order Now" /></div>
                <div className="space-y-2"><Label>CTA Button Link</Label><Input value={formData.cta_url} onChange={(e) => setFormData({ ...formData, cta_url: e.target.value })} className="bg-black border-white/20" placeholder="https://gameshopnepal.com/product/..." /></div>
              </div>
