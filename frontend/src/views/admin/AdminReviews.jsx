              ...
              <div key={review.id} className={`bg-card border rounded-lg p-4 hover:border-gold-500/30 transition-all ${review.status === 'pending' ? 'border-yellow-500/30' : review.status === 'rejected' ? 'border-red-500/20' : 'border-white/10'}`} data-testid={`review-row-${review.id}`}>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2 flex-wrap">
                      ...
                      {(review.is_verified_buyer || review.is_customer_review) && (
                        <Badge className="bg-green-500/20 text-green-400 border-green-500/30 text-xs">✅ Verified Buyer</Badge>
                      )}
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => { setReplyingTo(review.id); setReplyText(review.admin_reply || ''); }} className="text-white/60 hover:text-amber-500 p-2" title="Reply">
                  ...
                  {review.admin_reply && replyingTo !== review.id && (
                    <div className="mt-2 ml-2 pl-3 border-l-2 border-amber-500/30">
                      <p className="text-xs text-amber-400/80 font-semibold mb-0.5">Your Reply</p>
                      <p className="text-white/50 text-xs">{review.admin_reply}</p>
                    </div>
                  )}
                  ...
                  {replyingTo === review.id && (
                    <div className="mt-3 space-y-2">
                      <Textarea
                        ...
                        className="bg-black border-white/20 text-white placeholder:text-white/30 text-sm min-h-[80px] resize-none"
                      />
                      <div className="flex gap-2">
                        <Button size="sm" variant="ghost" onClick={() => setReplyingTo(null)} className="text-white/50">Cancel</Button>
                        <Button size="sm" onClick={() => handleReply(review.id)} className="bg-amber-500 hover:bg-amber-600 text-black font-semibold">
                          Post Reply
                        </Button>
                      </div>
                    </div>
