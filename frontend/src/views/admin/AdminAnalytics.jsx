  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      ...
      if (activeRange === 'today') {
        ...
        setRevenueChart((todayRes.data.hourly || []).map(item => ({
          ...item,
          rpv: item.visits > 0 ? Math.round(item.revenue / item.visits) : 0,
        })));
    ...
    <AdminLayout>
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          {/* Revenue per Visit (diagnoses traffic vs conversion separately) */}
          <Card className="bg-zinc-900 border-zinc-800 lg:col-span-2">
            <CardHeader className="pb-2">
              <CardTitle className="text-white text-base">Revenue per Visit <span className="text-zinc-500 text-xs font-normal ml-1">(monetization efficiency — rises = conversion fixing, falls = conversion problem)</span></CardTitle>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={260}>
                <LineChart data={revenueChart}>
                  ...
                  <XAxis dataKey={xAxisKey} stroke="#52525b" fontSize={11} tickLine={false} axisLine={false} tickFormatter={xAxisFormatter} interval={isToday ? 2 : 'preserveStartEnd'} />
                  <YAxis stroke="#52525b" fontSize={11} tickLine={false} axisLine={false} tickFormatter={(v) => `Rs ${v}`} />
                  <Tooltip {...chartTooltipStyle} formatter={(v) => [`Rs ${Math.round(v).toLocaleString()}`, 'Revenue per Visit']} labelFormatter={tooltipLabelFormatter} />
                  <Line type="monotone" dataKey="rpv" stroke="#F97316" strokeWidth={2} dot={{ fill: '#F97316', r: 3 }} activeDot={{ r: 5 }} />
                </LineChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
