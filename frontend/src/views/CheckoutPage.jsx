...
See invoice: ${invoiceUrl}`;
      ...
      clearCart();
      router.push(`/payment/${res.data.order_id}?${params.toString()}`);
    } catch (error) {
    ...
    <div className="min-h-screen bg-black">
      ...
      <main className="pt-14 md:pt-24 pb-20 md:pb-8">
        <div className="max-w-4xl mx-auto px-4">
          <Link href="/" className="inline-flex items-center text-gray-400 hover:text-amber-500 mb-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <div className="space-y-3">
                  {cart.map(item => (
                    <div key={`${item.product.id}-${item.variation.id}`} className="flex gap-3 p-3 bg-black/50 rounded-lg">
                      <img src={item.product.image_url} alt={item.product.name} className="w-12 h-12 rounded object-cover flex-shrink-0" />
                      <div className="flex-1 min-w-0">
                        <p className="text-white font-medium text-sm leading-snug break-words">{item.product.name}</p>
            ...
            <div className="lg:col-span-2">
              <form onSubmit={handleSubmitOrder} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 space-y-4 sticky top-24">
                ...
                {customer ? (
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <p className="text-amber-400/80 text-xs mt-1.5 flex items-center gap-1">📧 Your order details will be sent to this email — please make sure it's correct.</p>
                    </div>
                  </>
                )}
                <div>
                ...
                <Button type="submit" disabled={isSubmitting} data-testid="place-order-btn" className="w-full bg-amber-500 hover:bg-amber-600 text-black py-6">
